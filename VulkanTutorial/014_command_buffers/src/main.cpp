...
private:
    ...
    void initImgui() {
        ...
        init_info.RenderPass = renderPass;  // 必须设置 RenderPass
        init_info.Subpass = 0;
        ...
        // 上传字体纹理
        // 新版本的 ImGui 内部处理字体上传，不需要手动传递命令缓冲区
...
显示模式可以说是交换链最重要的设置，因为它代表了向屏幕显示图像的实际条件。Vulkan 中有四种可能的模式可用：
    ...
    void drawFrame() {
        ...
        // 自定义窗口示例
        {
            ImGui::Begin("Vulkan 教程");
            ImGui::Text("这是一个 ImGui 示例窗口");
            ImGui::Text("应用程序平均 %.3f ms/frame (%.1f FPS)",
