...
struct HelmetPush {
    ...
    int32_t debug_view { 0 }; ///< 0 PBR … 14 自发光，15 Base Color（见 IBL 面板）
...
static int run_pbr(int, char **) {
    ...
    // create_from_file 已 stbi_set_flip_vertically_on_load(1) 对齐行序，勿再对 UV 做 1−v，
    // 否则 Base Color 等与贴图预览会上下错开。
    ...
    while (running) {
            ...
            if (scene_tex_id != static_cast<ImTextureID>(0)) {
                ...
                if (ImGui::IsItemHovered()) {
                    ...
                    if (wheel != 0.0F) {
                        ...
                        cam.orbit_radius = std::clamp(cam.orbit_radius, k_orbit_min,
                                                      k_orbit_max);
                    }
                }
            }
        ...
        if (ImGui::Begin("IBL 预览")) {
            ...
            ImGui::SliderFloat("点光强度", &point_direct_strength, 0.0F, 6.0F,
                               "%.2f");
            ImGui::Separator();
            ImGui::Checkbox("分屏光照调试 (4×4，仅头盔)", &pbr_debug_tile_grid);
...

        {
            ...
            if (!pbr_debug_tile_grid) {
                ...
                hp.emissive_scale =
                    glm::vec4(emissive_scale, 0.0F, 0.0F, 0.0F);
                ...
                for (int ti = 0; ti < k_dbg_tile_count; ++ti) {
                    ...
                    glm::mat4 proj = glm::perspective(
                        glm::radians(55.0F), aspect, 0.05F, 50.0F);
                    proj[1][1] *= -1.0F;
