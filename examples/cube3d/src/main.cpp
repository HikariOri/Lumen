...
// 重建交换链 + 深度缓冲 + 帧缓冲
// 外部保证：调用前已经 vkDeviceWaitIdle
void recreateSwapchain(Swapchain &sw, lumen::platform::Window &window,
                       VkPhysicalDevice physDev, VkDevice device,
                       VkSurfaceKHR surface, VmaAllocator allocator,
                       VkRenderPass renderPass, VkFormat depthFormat) {
    sw.device = device;

    // 先销毁旧资源
    for (auto &fb : sw.framebuffers)
        vkDestroyFramebuffer(device, fb, nullptr);
    for (auto &v : sw.views)
        vkDestroyImageView(device, v, nullptr);
    if (sw.depthView)
        vkDestroyImageView(device, sw.depthView, nullptr);
    if (sw.depthImage)
        vmaDestroyImage(allocator, sw.depthImage, sw.depthAlloc);
    if (sw.handle)
        vkDestroySwapchainKHR(device, sw.handle, nullptr);

    sw.framebuffers.clear();
    sw.views.clear();

    // 获取窗口大小
    int w, h;
    window.get_framebuffer_size(&w, &h);
    sw.extent = { .width = (uint32_t)w, .height = (uint32_t)h };
    ...
    vkCreateSwapchainKHR(device, &sci, nullptr, &sw.handle);
    sw.imageFormat = sci.imageFormat;

    // 获取图像
    uint32_t count;
    vkGetSwapchainImagesKHR(device, sw.handle, &count, nullptr);
    sw.images.resize(count);
    vkGetSwapchainImagesKHR(device, sw.handle, &count, sw.images.data());

    // 创建 ImageView
    for (VkImage img : sw.images) {
    ...
    vmaCreateImage(allocator, &di, &allocInfo, &sw.depthImage, &sw.depthAlloc,
                   nullptr);
    ...
    vkCreateImageView(device, &dvi, nullptr, &sw.depthView);

    // 创建帧缓冲
    for (auto &v : sw.views) {
        VkImageView attachments[] = { v, sw.depthView };
...
int main() {
    ...
    auto context = *vulkan::Context::create(
        "cube3d", VK_MAKE_API_VERSION(0, 1, 0, 0),
    ...
    const auto vertexSpirv = load_spirv("./shaders/cube3d.vert.spv");
    const auto fragmentSpirv = load_spirv("./shaders/cube3d.frag.spv");
    ...
    // 描述符：set0=FrameUBO，set1=ObjectUBO（与 cube3d.vert 一致）
