...
namespace vulkan {
    ...
    class graphicsBase {
        ...
        ~graphicsBase() {
            ...
            if (debugMessenger) {
                PFN_vkDestroyDebugUtilsMessengerEXT
                    vkDestroyDebugUtilsMessenger = InstanceProcedureAddress(
        ...
        result_t CreateDebugMessenger() {
            ...
            PFN_vkCreateDebugUtilsMessengerEXT vkCreateDebugUtilsMessenger =
                InstanceProcedureAddress("vkCreateDebugUtilsMessengerEXT");
        ...
        static void ExecuteCallbacks(std::vector<void (*)()> &callbacks) {
            for (size_t size = callbacks.size(), i = 0; i < size; i++)
                callbacks[i]();
            // for (auto& i : callbacks) i(); //Not safe for (size_t i = 0; i <
            // callbacks.size(); i++) callbacks[i]();//Not safe
        }
        ...
        result_t CheckDeviceExtensions(arrayRef<const char *> extensionsToCheck,
            ...
            if (extensionCount) {
                ...
                for (auto &i : extensionsToCheck) {
                    ...
                    if (!found)
                        i = nullptr; // If a required extension isn't available,
                                     // set it to nullptr
                }
            } else
                for (auto &i : extensionsToCheck)
        ...
        result_t CreateSwapchain(bool limitFrameRate = true,
            ...
            if (!availableSurfaceFormats.size())
                if (VkResult result = GetSurfaceFormats())
                    return result;
            // If surface format is not determined, select a four-component
            // UNORM format
    ...
    public:
        ...
        VkMemoryAllocateInfo MemoryAllocateInfo(
            ...
            vkGetBufferMemoryRequirements(graphicsBase::Base().Device(), handle,
                                          &memoryRequirements);
            ...
            for (size_t i = 0;
                 i < physicalDeviceMemoryProperties.memoryTypeCount; i++)
            ...
            // 不在此检查是否成功取得内存类型索引，因为会把 memoryAllocateInfo
            // 返回出去，交由外部检查
            //
            // if (memoryAllocateInfo.memoryTypeIndex == UINT32_MAX)
            //     outStream << std::format("[ buffer ] ERROR\nFailed to find
            //     any memory type satisfies all desired memory properties!\n");
            return memoryAllocateInfo;
        }

        result_t BindMemory(VkDeviceMemory deviceMemory,
                            VkDeviceSize memoryOffset = 0) const {
            VkResult result =
                vkBindBufferMemory(graphicsBase::Base().Device(), handle,
                                   deviceMemory, memoryOffset);
            if (result) {
                std::println("[ buffer ] ERROR\nFailed to attach the "
                             "memory!\nError code: {}\n",
                             string_VkResult(result));
            }
            return result;
        ...
        result_t Create(VkBufferCreateInfo &createInfo) {
            ...
            VkResult result = vkCreateBuffer(graphicsBase::Base().Device(),
                                             &createInfo, nullptr, &handle);
            if (result) {
                std::println("[ buffer ] ERROR\nFailed to create a "
                             "buffer!\nError code: {}\n",
    ...
    public:
        ...
        Create(VkBuffer buffer, VkFormat format, VkDeviceSize offset = 0,
               VkDeviceSize range = 0 /*VkBufferViewCreateFlags flags*/) {
            VkBufferViewCreateInfo createInfo = { .buffer = buffer,
                                                  .format = format,
                                                  .offset = offset,
                                                  .range = range };
            return Create(createInfo);
    ...
    public:
        ...
        VkMemoryAllocateInfo MemoryAllocateInfo(
            ...
            VkMemoryRequirements memoryRequirements;
            vkGetImageMemoryRequirements(graphicsBase::Base().Device(), handle,
                                         &memoryRequirements);
            memoryAllocateInfo.allocationSize = memoryRequirements.size;

            auto GetMemoryTypeIndex =
                [](uint32_t memoryTypeBits,
                   VkMemoryPropertyFlags desiredMemoryProperties) -> uint32_t {
                auto &physicalDeviceMemoryProperties =
                    graphicsBase::Base().PhysicalDeviceMemoryProperties();
                for (size_t i = 0;
                     i < physicalDeviceMemoryProperties.memoryTypeCount; i++)
            ...
            //     memory type satisfies all desired memory properties!\n");
        ...
        result_t BindMemory(VkDeviceMemory deviceMemory,
                            VkDeviceSize memoryOffset = 0) const {
            VkResult result =
                vkBindImageMemory(graphicsBase::Base().Device(), handle,
                                  deviceMemory, memoryOffset);
            if (result) {
                std::println("[ image ] ERROR\nFailed to attach the "
                             "memory!\nError code: {}\n",
                             string_VkResult(result));
            }
            return result;
        ...
        result_t Create(VkImageCreateInfo &createInfo) {
            ...
            VkResult result = vkCreateImage(graphicsBase::Base().Device(),
                                            &createInfo, nullptr, &handle);
            if (result) {
                std::println("[ image ] ERROR\nFailed to create an "
                             "image!\nError code: {}\n",
    ...
    public:
        ...
        result_t Create(VkImageViewCreateInfo &createInfo) {
            ...
            VkResult result = vkCreateImageView(graphicsBase::Base().Device(),
                                                &createInfo, nullptr, &handle);
            if (result) {
                std::println("[ imageView ] ERROR\nFailed to create an image "
                             "view!\nError code: {}\n",
                             string_VkResult(result));
            }
            return result;
        ...
        result_t Create(VkImage image, VkImageViewType viewType,
                        VkFormat format,
                        const VkImageSubresourceRange &subresourceRange,
                        VkImageViewCreateFlags flags = 0) {
    ...
    public:
        ...
        result_t Create(VkDescriptorSetLayoutCreateInfo &createInfo) {
            ...
            VkResult result = vkCreateDescriptorSetLayout(
                graphicsBase::Base().Device(), &createInfo, nullptr, &handle);
            if (result) {
                std::println("[ descriptorSetLayout ] ERROR\nFailed to create "
    ...
    public:
        ...
        void Write(arrayRef<const bufferView> descriptorInfos,
                   VkDescriptorType descriptorType, uint32_t dstBinding = 0,
                   uint32_t dstArrayElement = 0) const {
            Write({ descriptorInfos[0].Address(), descriptorInfos.Count() },
                  descriptorType, dstBinding, dstArrayElement);
        }

        // Static Function
        static void Update(arrayRef<VkWriteDescriptorSet> writes,
                           arrayRef<VkCopyDescriptorSet> copies = {}) {
            ...
            vkUpdateDescriptorSets(graphicsBase::Base().Device(),
                                   writes.Count(), writes.Pointer(),
                                   copies.Count(), copies.Pointer());
        }
    };
    ...
    public:
        ...
        result_t
        AllocateSets(arrayRef<VkDescriptorSet> sets,
                     arrayRef<const VkDescriptorSetLayout> setLayouts) const {
            if (sets.Count() != setLayouts.Count())
                if (sets.Count() < setLayouts.Count()) {
                    std::println(
                        "[ descriptorPool ] ERROR\nFor each descriptor set, "
                        "must provide a corresponding layout!\n");
                    return VK_RESULT_MAX_ENUM; // 没有合适的错误代码，别用VK_ERROR_UNKNOWN
                } else {
                    std::println("[ descriptorPool ] WARNING\nProvided layouts "
                                 "are more than sets!\n");
                }
            ...
            VkResult result = vkAllocateDescriptorSets(
                graphicsBase::Base().Device(), &allocateInfo, sets.Pointer());
            if (result) {
                std::println("[ descriptorPool ] ERROR\nFailed to allocate "
                             "descriptor sets!\nError code: {}\n",
                             string_VkResult(result));
            }
        ...
        result_t
        AllocateSets(arrayRef<VkDescriptorSet> sets,
                     arrayRef<const descriptorSetLayout> setLayouts) const {
            return AllocateSets(
                sets, { setLayouts[0].Address(), setLayouts.Count() });
        }

        result_t
        AllocateSets(arrayRef<descriptorSet> sets,
                     arrayRef<const VkDescriptorSetLayout> setLayouts) const {
        ...
        result_t FreeSets(arrayRef<VkDescriptorSet> sets) const {
            VkResult result =
                vkFreeDescriptorSets(graphicsBase::Base().Device(), handle,
                                     sets.Count(), sets.Pointer());
            if (result) {
                std::println("[ descriptorPool ] ERROR\nFailed to free "
                             "descriptor sets!\nError code: {}\n",
                             string_VkResult(result));
            } else {
                memset(sets.Pointer(), 0,
                       sets.Count() * sizeof(VkDescriptorSet));
            }
            return result;
        }
        ...
        result_t Reset(/*VkDescriptorPoolResetFlags flags*/) const {
            return vkResetDescriptorPool(graphicsBase::Base().Device(), handle,
                                         0);
        }
        ...
        result_t Create(uint32_t maxSetCount,
                        arrayRef<const VkDescriptorPoolSize> poolSizes,
                        VkDescriptorPoolCreateFlags flags = 0) {
    ...
    public:
        ...
        result_t Create(VkSamplerCreateInfo &createInfo) {
            ...
            VkResult result = vkCreateSampler(graphicsBase::Base().Device(),
                                              &createInfo, nullptr, &handle);
            if (result) {
                std::println("[ sampler ] ERROR\nFailed to create a "
                             "sampler!\nError code: {}\n",
                             string_VkResult(result));
            }
            return result;
    ...
    public:
        ...
        void CmdReset(VkCommandBuffer commandBuffer, uint32_t firstQueryIndex,
                      uint32_t queryCount) const {
            vkCmdResetQueryPool(commandBuffer, handle, firstQueryIndex,
                                queryCount);
        }

        void CmdBegin(VkCommandBuffer commandBuffer, uint32_t queryIndex,
        ...
        void CmdWriteTimestamp(VkCommandBuffer commandBuffer,
            ...
            vkCmdWriteTimestamp(commandBuffer, pipelineStage, handle,
                                queryIndex);
        }

        void CmdCopyResults(VkCommandBuffer commandBuffer,
        ...
        result_t GetResults(uint32_t firstQueryIndex, uint32_t queryCount,
                            size_t dataSize, void *pData_dst,
                            VkDeviceSize stride,
                            VkQueryResultFlags flags = 0) const {
            VkResult result = vkGetQueryPoolResults(
                graphicsBase::Base().Device(), handle, firstQueryIndex,
                queryCount, dataSize, pData_dst, stride, flags);

            if (result) {
                result > 0
                    ? // 若返回值为VK_NOT_READY，则查询尚未结束，有查询结果尚不可获
                    std::println("[ queryPool ] WARNING\nNot all queries are "
                                 "available!\nError code: {}\n",
                                 string_VkResult(result))
                    : std::println("[ queryPool ] ERROR\nFailed to get query "
                                   "pool results!\nError code: {}\n",
                                   string_VkResult(result));
            }
        ...
        void Reset(uint32_t firstQueryIndex, uint32_t queryCount) const {
            vkResetQueryPool(graphicsBase::Base().Device(), handle,
                             firstQueryIndex, queryCount);
        }

        // Non-const Function
        result_t Create(VkQueryPoolCreateInfo &createInfo) {
            createInfo.sType = VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO;
            VkResult result = vkCreateQueryPool(graphicsBase::Base().Device(),
                                                &createInfo, nullptr, &handle);
            if (result) {
        ...
        result_t Create(VkQueryType queryType, uint32_t queryCount,
                        VkQueryPipelineStatisticFlags pipelineStatistics = 0,
                        VkQueryPoolCreateFlags flags = 0) {
            VkQueryPoolCreateInfo createInfo = { .flags = flags,
                                                 .queryType = queryType,
    ...
    public:
        ...
        result_t GetResults() { return GetResults(Capacity()); }

        result_t GetResults(uint32_t queryCount) {
            return queryPool.GetResults(0, queryCount, queryCount * 4,
                                        passingSampleCounts.data(), 4);
        }
    };
    ...
    public:
        ...
        timestampQueries(uint32_t capacity) { Create(capacity); }
        ...
        void CmdWriteTimestamp(VkCommandBuffer commandBuffer,
                               VkPipelineStageFlagBits pipelineStage,
                               uint32_t queryIndex) const {
            queryPool.CmdWriteTimestamp(commandBuffer, pipelineStage,
                                        queryIndex);
        }
