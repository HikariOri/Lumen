/**
 * @file main.cpp
 * @brief 索引绘制矩形：4 顶点 + uint16 索引（两枚三角形）
 */
...
static int run_triangle() {
    ...
    const std::string vertPath =
        lumen::core::get_resource_path("shaders/rectangle_indexed.vert.spv");
    const std::string fragPath =
        lumen::core::get_resource_path("shaders/rectangle_indexed.frag.spv");
    ...
    LUMEN_APP_LOG_INFO("矩形（索引绘制）运行中，按 Esc 退出");
    ...
    while (running) {
        ...
        vkCmdDrawIndexed(cmdBuffers[currentFrame], indices.size(), 1, 0, 0, 0);
