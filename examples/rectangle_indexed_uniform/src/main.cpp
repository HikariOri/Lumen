...
/// UBO 与 `rectangle_indexed_uniform.vert` 中 `vec4 data` 一致（std140 友好）
constexpr size_t kTimeUboSize { sizeof(glm::vec4) };

/// 角速度（弧度/秒），与 UBO 的 `data.y` 一致，顶点着色器用 `data.x * data.y`
/// 作为转角
constexpr float kRotationRadPerSecond { 1.2F };
...
static int run_rectangle_textured() {
    ...
    winConfig.title = "Lumen — 索引矩形 + UBO 时间旋转";
    winConfig.width = 800;
    ...
    const std::string vertPath = lumen::core::get_resource_path(
        "shaders/rectangle_indexed_uniform.vert.spv");
    const std::string fragPath = lumen::core::get_resource_path(
        "shaders/rectangle_indexed_uniform.frag.spv");
    ...
    std::array<lumen::render::UniformBuffer, kMaxFramesInFlight>
        timeUniforms {};
    for (auto &ub : timeUniforms) {
        if (!ub.create_persistent(ctx, kTimeUboSize)) {
            LUMEN_APP_LOG_ERROR("Time UniformBuffer 创建失败");
            return -1;
        }
    }
