...
private:
    ...
    void createFramebuffers() {
        ...
        for (size_t i = 0; i < swapChainImageViews.size(); ++i) {
            // 定义 framebuffer 的 attachments，这里只有一个
            // attachment，就是当前的 swapChainImageView
            VkImageView attachments[] = { swapChainImageViews[i] };

            // 准备 VkFramebufferCreateInfo 结构，描述 framebuffer 的参数
            VkFramebufferCreateInfo framebufferInfo {};
            framebufferInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
            // renderPass 是之前创建好的 render pass，framebuffer 必须和这个
            // render pass 兼容
            ...
            // 图像层数 (layers)，这里是单层 (1)，因为 swapchain image
            // 通常是单层 2D 图像
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
            ...
            descriptorWrite.dstBinding = 0; // 对应 layout 中哪个 binding（假设
                                            // uniform buffer 在 binding 0）
            descriptorWrite.dstArrayElement =
                0; // 如果 binding 是数组，这里是数组起始索引。我们不是数组，用
                   // 0。
