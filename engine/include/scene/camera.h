...
namespace lumen {
    ...
    struct Camera {
        ...
        glm::mat4 getViewMatrix() const {
            ...
            // 假设 rotation.x = yaw (绕 Y), rotation.y = pitch (绕 X),
            // rotation.z = roll (绕 Z) 先把欧拉角转成弧度
            ...
            // translate(-position)
            // 或者这么做：先旋转相机（相机的方向），再移动相机
            view =
                glm::inverse(rot) * glm::translate(glm::mat4(1.0f), -position);
            return view;
        }
        ...
        // 设置为正交投影
        void setOrthographic(float width, float height, float nearC,
                             float farC) {
