...
void pack_lights_for_ubo(const ::entt::registry &registry, GPULight *out_lights,
    ...
    for (const ::entt::entity e : registry.view<DirectionalLightComponent>()) {
        ...
        glm::vec3 surf_to_light =
            linear * kDefaultSurfToLightLocal;
        surf_to_light =
        ...
        items.push_back(
            Item { .sort_key = static_cast<std::uint32_t>(::entt::to_integral(e)),
                   .gpu = g });
    }
    ...
    for (const ::entt::entity e : registry.view<PointLightComponent>()) {
        ...
        const glm::mat4 world = world_matrix(registry, e);
        const float range =
            std::max(std::max(pl.radius, pl.minRadius), 1e-4f);

        GPULight g {};
        g.position =
            glm::vec4(world_translation(world), 1.0f);
