...
bool Context::init_instance(const ContextConfig &config) {
  ...
  if (result != VK_SUCCESS) {
    LUMEN_LOG_ERROR("vkCreateInstance failed: {}", static_cast<int>(result));
    return false;
  }
  return true;
}
...
bool Context::pick_physical_device_(VkSurfaceKHR surface) {
  ...
  for (VkPhysicalDevice dev : devices) {
    ...
    if (gfxIdx != UINT32_MAX &&
      ...
      presentQueueFamily_ = surface != VK_NULL_HANDLE ? presentIdx : gfxIdx;

      vkGetPhysicalDeviceProperties(dev, &physicalDeviceProperties_);
      vkGetPhysicalDeviceFeatures(dev, &physicalDeviceFeatures_);
...
PhysicalDeviceInfo Context::physical_device_info() const {
  ...
  const auto &p = physicalDeviceProperties_;
  ...
  case VK_PHYSICAL_DEVICE_TYPE_CPU:
    ...
    break;
  default:
    info.deviceType = PhysicalDeviceType::Other;
    break;
  }
