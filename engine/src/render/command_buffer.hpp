 ...
 * @brief CommandPool、帧同步聚合入口
 *
 * Vulkan 中所有 GPU 操作都需通过命令缓冲提交，
 ...
 * - `vk::CommandBuffer`：用来 *记录 GPU 要执行的命令*（直接使用 Vulkan-Hpp：
 *   `reset`、`begin`、`end`、各类 `vkCmd*` 对应成员）。
 ...
 * @brief Vulkan 命令池
 *
 * CommandPool 用于分配 `vk::CommandBuffer`。在 Vulkan 里命令缓冲来自池，
 * 池的生命周期应与命令缓冲一致。命令池通常按 **队列族索引** 创建，
 * 因为不同队列族（graphics/compute/transfer）无法复用同一个池。
 *
 * 使用流程：
 * 1. 创建 VkCommandPool
 * 2. 从 pool allocate N 个 `vk::CommandBuffer`
 * 3. 对缓冲调用 `reset`（若复用）、`begin`（填写 `vk::CommandBufferBeginInfo`）、
 *    录制、`end`
...
public:
    ...
    /**
     * @brief 分配 primary / secondary 命令缓冲
     * @param count 数量
     * @param level `vk::CommandBufferLevel::ePrimary` / `eSecondary`
     * @return 成功返回句柄列表
     *
    ...
    /**
     * @brief 释放由此命令池分配的 command buffer
     */
    void free(const std::vector<vk::CommandBuffer> &buffers);
     ...
     * `vk::CommandBufferUsageFlagBits::eOneTimeSubmit` 调用 `begin`、录制、`end`、
     * 提交并等待完成
     *
     ...
     * @param record 录制闭包（在成功的 `begin` 与 `end` 之间调用）
     * @return 任一步失败返回 false（buffer 仍会释放）
     */
    bool submit_one_shot(
        vk::Queue queue,
        const std::function<void(vk::CommandBuffer cmd)> &record);

    [[nodiscard]] vk::CommandPool handle() const { return pool_; }
