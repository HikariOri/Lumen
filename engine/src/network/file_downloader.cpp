...
// ========== ExecuteRequest 实现 ==========
DownloadResponse FileDownloader::ExecuteRequest(
    const std::string &url, const std::string &method,
    const std::optional<std::string> &body, const DownloadOptions &options) {
    ...
    while (true) {
        try {
            ...
            if (method == "GET") {
                ...
                response =
                    cpr::Post(cpr_url, headers, body_data, timeout, verify_ssl);
            } else if (method == "PUT") {
                cpr::Body body_data { body.value_or("") };
                response =
                    cpr::Put(cpr_url, headers, body_data, timeout, verify_ssl);
            } else if (method == "DELETE") {
                response = cpr::Delete(cpr_url, headers, timeout, verify_ssl);
            } else {
...
bool FileDownloader::DownloadToFile(const std::string &url,
    ...
    try {
        ...
        if (!file.is_open()) {
            LUMEN_LOG_ERROR("Failed to open file for writing: {}", filepath);
            return false;
        }
...
// 高级版本（带options）
DownloadResponse FileDownloader::DownloadText(const std::string &url,
                                              const DownloadOptions &options) {
...
FileDownloader::DownloadTextAsync(const std::string &url,
    ...
    return std::async(std::launch::async,
                      [url, options]() { return DownloadText(url, options); });
}
...
void FileDownloader::DownloadToMemoryAsync(const std::string &url,
