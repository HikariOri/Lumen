...
void Texture2D::loadFromFile(const std::string_view &path,
    ...
    mipLevels = static_cast<std::uint32_t>(std::floor(std::log2(static_cast<float>(dim)))) + 1U;
    ...
    // Mipmap：blit 要求源为 TRANSFER_SRC_OPTIMAL，目标为 TRANSFER_DST_OPTIMAL（同一张图需逐层 barrier）
    for (std::uint32_t i = 1; i < mipLevels; ++i) {
        ...
        const int32_t srcW = std::max(1, static_cast<int32_t>(width >> (i - 1)));
        const int32_t srcH = std::max(1, static_cast<int32_t>(height >> (i - 1)));
