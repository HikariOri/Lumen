 ...
 * @brief 由 `RenderTargetBundle` 描述构建的简化图形 `VkRenderPass`（多颜色 + 可选深度）。
 */
 ...
 *
 * @note 与 `RenderTargetBundle::get_framebuffer(device, vk_render_pass())` 配套使用；
 * 附件顺序须与 bundle 一致（颜色 → 深度）。
 */
class RenderPass final {
public:
    [[nodiscard]] static std::expected<RenderPass, std::string>
    create(VkDevice device, const RenderTargetBundle &bundle,
           bool color_final_present_src = false);
