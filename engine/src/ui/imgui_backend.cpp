...
bool imgui_backend_init(const ImGuiBackendInitInfo &info) {
    ...
    lumen::asset::AssetRegistry::instance().fonts().setup_imgui_application_fonts(
        io, info.cjk_font_ttf_path, info.cjk_font_japanese_merge_path,
...
void imgui_backend_shutdown() {
    ImGui_ImplVulkan_Shutdown();
    ImGui_ImplSDL3_Shutdown();
    lumen::asset::AssetRegistry::instance().fonts().clear_session();
    ImGui::DestroyContext();
}
